...
<h4 class="header-title mb-6">{{__('backend.update_post')}} -  {{__('backend.'.$_GET['main_category'])}} - {{__('backend.'.$_GET['category'])}} - {{$data->title}} </h4>
{!! Form::model( $data ,[ 'url' =>  config('settings.BackendPath').'/posts/'.$data->id, 'method'=>'PATCH' ,  'class' => 'form' ,  'files' => 'true' ]) !!}
